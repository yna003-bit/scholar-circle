...
export default async function UserFollowingPage({ params }: { params: { userId: string } }) {
  ...
  return (
    <div>
      ...
      <h1 className="mb-4 text-lg font-medium">
        {params.userId === user.id ? "Following" : `Who ${profile.display_name} follows`}
      </h1>
      <div className="flex flex-col gap-2">
        ...
        {people.length === 0 ? (
          <p className="text-sm text-ink/50 dark:text-neutral-400">Not following anyone yet.</p>
        ) : null}
      </div>
    </div>
  );
